...
export default class MeasureTool {
			...
			this.MeasureEntities.push(
				this.viewer.entities.add({
					...
					label: {
						...
						backgroundColor: new Color.fromCssColorString(
							"#c6c691",
						),
					},
				}),
	...
	drawLine(height) {
		...
		this.handler.setInputAction((movement) => {
			...
			if (!cartesian) return;
			let cartesianHeight = cartesianWithHeight(cartesian, height);
			if (positions.length == 0) {
				// 初始存储两点 便于move事件使用
				positions.push(cartesianHeight);
				positions.push(cartesianHeight.clone());
				movePositions.push(cartesianHeight);
				movePositions.push(cartesianHeight.clone());
				// 添加初始点entity
				this.MeasureEntities.push(
					this.viewer.entities.add({
						position: cartesianHeight,
						point: {
							pixelSize: 10,
							color: Color.GREEN,
							disableDepthTestDistance: Number.POSITIVE_INFINITY,
		...
		this.handler.setInputAction((movement) => {
			...
			let cartesianHeight = cartesianWithHeight(cartesian, height);
			if (positions.length == 0) {
				this.CesiumTooltip.showAt(cartesianHeight, "点击添加第一个点");
			} else if (positions.length >= 2) {
				this.CesiumTooltip.showAt(cartesianHeight, "右击结束测量");
				if (!defined(polyLine)) {
	...
	drawPolygon(height) {
		...
		this.handler.setInputAction((movement) => {
			...
			if (positions.length == 0) {
				...
				positions.push(cartesianHeight.clone());
				// 真实点存储
				polyGonPoints.push(cartesianHeight);
				// 添加点
				this.MeasureEntities.push(
					this.viewer.entities.add({
		...
		this.handler.setInputAction((movement) => {
			...
			if (positions.length == 0) {
				...
				if (polyGonPoints.length == 1) {
					this.CesiumTooltip.showAt(
						cartesianHeight,
						"点击添加第二个点",
					);
				} else {
					this.CesiumTooltip.showAt(cartesianHeight, "右击结束");
				}
				if (!defined(polyGon)) {
	...
	drawCircle(height) {
		...
		this.handler.setInputAction((movement) => {
			...
			if (clickNum == 0) {
				...
				this.CesiumTooltip.showAt(cartesianHeight, "点击完成绘制");
				if (!defined(circle)) {
		...
		this.MeasureEntities.push(
			this.viewer.entities.add({
				...
				label: {
					...
					fillColor: Color.GREEN,
					pixelOffset: new Cartesian2(80, -15),
					horizontalOrigin: HorizontalOrigin.RIGHT,
					scale: 0.5,
					showBackground: true,
					backgroundPadding: new Cartesian2(15, 10),
					backgroundColor: new Color.fromCssColorString("#c6c691"),
				},
			}),
			this.viewer.entities.add({
				position: endPoint,
				label: {
					...
					pixelOffset: new Cartesian2(72, -10),
					horizontalOrigin: HorizontalOrigin.RIGHT,
					scale: 0.5,
	...
	constructor(option) {
		...
		this.labelPoint = this.viewer.entities.add({
			...
			position: new CallbackProperty(() => {
				let center = [];
				if (this.positions.length >= 3) {
					center = BoundingSphere.fromPoints(this.positions).center;
				}
				return center;
			}, false),
			label: {
				...
				text: new CallbackProperty(() => {
					...
					if (this.positions.length >= 3) {
						let area = parseInt(
							countAreaInCartesian3(this.positions),
						);
...
function cartesian3ToDegrees(cartesian) {
	...
	return { text: text, cartesian: cartesian };
}
/**
 * @Author: dongnan
